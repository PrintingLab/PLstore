@extends('layouts.app')
@section('title')
Upload File product - Printing Lab
@endsection
@section('insert_css')
...
<div class="container textoscontainer">
  ...
  <div class="divfile">
    @if ($face=='2')
    <form action="{{route('cart')}}" method="post" accept-charset="utf-8" enctype="multipart/form-data">
      {{csrf_field()}}
      ...
      <!-- declaración de variable que determina origen de la imagen  -->
      <input type="hidden" name="caras" value="2">
...
@else
<form action="{{route('cart')}}" method="post" accept-charset="utf-8" onsubmit="return validatevacio();" enctype="multipart/form-data" >
  {{csrf_field()}}
  <input type="hidden" name="idP" value="{{$id_products}}">
  <input type="hidden" name="idC" value="{{$id_category}}">
  <input type="hidden" name="idE" value="{{$id_specs}}">
  <!-- declaración de variable que determina origen de la imagen  -->
  <input type="hidden" name="caras" value="1">
  <div class="row">
    <div class="col-md-6">
      <h5><strong>UPLOAD AN EXANPLE OF WHAT YOU WANT:</strong></h5>
      <p><i>Only JPG,PNG,PDF,DOC,PPT</i></p>
